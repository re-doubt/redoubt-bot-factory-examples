...
async def handler(obj):
    ...
    if obj['data']['amount'] >= TRANSFER_THRESHOLD:
        ...
        if obj['data']['source'] in address_labels.keys():
            source = address_labels[obj['data']['source']]
        else:
            source = obj['data']['source'][:2]+ '..' +obj['data']['source'][-4:]

        if obj['data']['destination'] in address_labels.keys():
            destination = address_labels[obj['data']['destination']]
        else:
...
async def run_bot():
    logger.info("Running new Large Transfer Out bot")
